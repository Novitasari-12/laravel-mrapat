...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-5">
                    <form action="{{route('informasi_gambar.update', [$igambar->id])}}" enctype="multipart/form-data" method="post">
                        {{ csrf_field() }}
                        {{method_field('PUT')}}
                        <div class="form-group">
                            <label class="block clearfix" >Nama Gambar</label>
                            <input type="text" name='nama' value="{{$igambar->nama}}" class="form-control form-control-sm  {{ $errors->has('nama') ? 'is-invalid' : '' }}" id="nama" placeholder="Nama Gambar">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control form-control-sm  {{ $errors->has('informasi_gambar') ? 'is-invalid' : '' }}" name="informasi_gambar" placeholder="Gambar">{{$igambar->informasi_gambar}}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local"  value="{{str_replace(' ', 'T', date('Y-m-d H:i:s', strtotime($igambar->waktu_mulai_ditampilkan)))}}" class="form-control form-control-sm  {{ $errors->has('waktu_mulai_ditampilkan') ? 'is-invalid' : '' }}" name="waktu_mulai_ditampilkan" placeholder="Waktu Mulai Ditampilkan">
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" value="{{str_replace(' ', 'T', date('Y-m-d H:i:s', strtotime($igambar->waktu_selesai_ditampilkan)))}}" class="form-control form-control-sm  {{ $errors->has('waktu_selesai_ditampilkan') ? 'is-invalid' : '' }}" name="waktu_selesai_ditampilkan" placeholder="Waktu Selesai Ditampilkan">
                ...
                <div class="col-md-4">
                    <img src="{{$igambar->gambar}}" class="col-md-12" id="out-image" />
                </div>
            </div>
        </div>
    </div>
